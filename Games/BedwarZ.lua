            ...
            Aura:Start(function()
                if isAlive(lplr) then
                    ...
                    if Entity then
                        ...
                        if Sword and (TimedHits.Enabled and (tick() - lastAttacked) > 0.01 or true) then
                            loadAnim:Stop()
                            loadAnim:Play()
        ...
        if callback then
            Flight:Start(function(deltaTime: number)
                if isAlive(lplr) then
                    ...
                    root.Velocity = Vector3.new(velo.X, setY, velo.Z)
                end
            end)
        end
    end
    ...
    ['Function'] = function(callback)
        if callback then
            Scaffold:Start(function()
                local Wool = getItem('wool') or getHoldingItem('wool') or getItem('block') or getHoldingItem('block')
                if Wool then
                    local Pos = roundPos(lplr.Character.PrimaryPart.Position - Vector3.new(0, 3, 0) + lplr.Character.PrimaryPart.CFrame.LookVector * 3)
...
local charAdded
local AnticheatBypass = Utility:CreateModule({
